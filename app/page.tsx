...
import { 
  ...
  ExternalLink, Sparkles, Trophy, Github, Globe
} from 'lucide-react'
...
export default function PublicProfile() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        // 1. Get Logged in User (to see if they can vouch)
        const { data: { session } } = await supabase.auth.getSession()
        if (session) setCurrentUser(session.user)

        // 2. Fetch all vouches for counting
        ...
        // 3. Fetch user's vouches if logged in
        if (session) {
  ...
  const handleVouch = async (projectId) => {
    if (!currentUser?.id || vouchedIds.includes(projectId)) return
      ...
      <div className="max-w-[1000px] mx-auto px-4 md:px-8 pt-12 pb-20">
        ...
        <div className="bg-[#151821] border border-white/5 rounded-[2.5rem] p-8 md:p-12 mb-12 relative overflow-hidden flex flex-col md:flex-row items-center md:items-start gap-8">
           ...
           <div className="flex-1 text-center md:text-left z-10">
              ...
              {/* Profile Stats */}
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 border-t border-white/5 pt-6">
                 ...
                 <div><p className="text-2xl font-bold text-blue-400">{Object.values(globalVouchCounts).reduce((a, b) => a + b, 0)}</p><p className="text-xs text-gray-500 uppercase tracking-wider font-bold">Total Vouches</p></div>
              </div>
           </div>
        </div>
...
function ProfileProjectCard({ id, title, tag, skills, desc, link, vouchCount, onVouch, vouched, created_at, image_url, currentUser }) {
  // Read-Only version of the Feed Card for the public profile
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#151821] border border-white/5 rounded-[2rem] p-6 sm:p-8 hover:border-white/10 transition-colors shadow-xl shadow-black/50">
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5">
         <div className="flex items-center gap-6">
            <button onClick={onVouch} disabled={vouched || !currentUser} className={`flex items-center gap-2 text-sm font-medium transition-colors ${vouched ? 'text-blue-500 cursor-default' : currentUser ? 'text-gray-400 hover:text-white' : 'text-gray-600 cursor-not-allowed'}`}>
               <Heart size={18} className={vouched ? 'fill-blue-500' : ''} />
               <span>{vouchCount}</span>
            </button>
            {/* Note: In a real app, clicking this on the profile page could link back to the main feed to open the thread */}
